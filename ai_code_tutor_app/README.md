...
- **Build Studio projects**: four guided builds (Quiz Scorekeeper, JSON Habit Tracker, Prompt Coach, Text Analyzer) where you grow one real program step by step in an in-app editor — each step has lesson-linked instructions, AST-based structure checks that work even with the code runner off, hints, catch-up code, and a download of your finished `.py`. A fifth **Design Your Own** track lets you scope your own idea (with optional AI scoping help), pick the Python ingredients it must use, and verify your code against your own recipe. Checkpoint tracks and **streaks, XP, levels, and badges** round it out.
